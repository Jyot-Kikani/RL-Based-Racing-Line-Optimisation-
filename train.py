...
STAGES = {
    1: {
        "name": "Survival - Custom F1",
        "track": "data/tracks/f1.csv",
        "reward_mode": "checkpoint",
        "steps": 400_000,
        "lr": 3e-4,
        "description": "Learn to survive a full lap on the custom F1 track.",
    },
    2: {
        "name": "Time Attack - Custom F1",
        "track": "data/tracks/f1.csv",
        "reward_mode": "laptime",
        "steps": 500_000,
        "lr": 1e-4,
        "description": "Optimise lap time on the same custom F1 track.",
    },
    3: {
        "name": "Fine Tune - Custom F1",
        "track": "data/tracks/f1.csv",
        "reward_mode": "laptime",
        "steps": 300_000,
        "lr": 5e-5,
        "description": "Low-LR refinement of the learned racing line on the custom F1 track.",
    },
}
...
def train(stage: int, steps_override: int = None, n_envs: int = 1, headless: bool = False):
    ...
    os.makedirs(os.path.join(LOG_DIR, f"stage{stage}"), exist_ok=True)
    ...
    # SB3 counts eval/checkpoint frequency in per-env steps when using VecEnv,
    # so scale them down to keep roughly the same wall-clock cadence.
    scaled_eval_freq = max(1_000, 10_000 // n_envs)
    scaled_ckpt_freq = max(1_000, 5_000 // n_envs)
    ...
    if stage > 1 and os.path.exists(prev_path):
        ...
        same_track_as_prev = STAGES[stage]["track"] == STAGES[stage - 1]["track"]
        if same_track_as_prev and os.path.exists(buf_path):
